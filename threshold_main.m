...
threshold = 0:20:100; %Threshold for grouping APs

%Storing rate for each instance.
sum_MonteCarloMRT = zeros(K,length(threshold));
sum_MonteCarloFZF = zeros(K,length(threshold));
sum_MonteCarloPZF = zeros(K,length(threshold));
sum_MonteCarloPPZF = zeros(K,length(threshold));
sum_closedForm_MRT = zeros(K,length(threshold));
sum_closedForm_FZF = zeros(K,length(threshold));
sum_closedForm_PZF = zeros(K,length(threshold));
sum_closedForm_PPZF = zeros(K,length(threshold));
...
closedForm_PPZF = zeros(lsf_experiment,K);
    ...
    for ic=1:lsf_experiment
     ...
     [S,W,Z,X,E_S_temp,tau_S_l] = functionUEgrouping(M,beta_matrix,Pilot_indices,tau,threshold(id)/100); %UE grouping

     [rate_sim_MRT,rate_sim_FZF,rate_sim_PZF,rate_sim_PPZF] = functionSimulation(L,M,K,p_maxdB,ppdB,beta_matrix,Phii,tau,Pilot_indices,pilot_book,S,W,Z,X,E_S_temp,tau_S_l,experiment); %Sum rate 
...
disp(['Setup ' num2str(id) ' out of ' num2str(length(threshold))]);
end
disp('Execution Complete !!!')

figure(3);
hold on;
plot(threshold,mean(sum_MonteCarloMRT),'-ob','LineWidth', 1.2);
plot(threshold,mean(sum_MonteCarloFZF),'-or','LineWidth', 1.2);
plot(threshold,mean(sum_MonteCarloPZF),'-ok','LineWidth', 1.2);
plot(threshold,mean(sum_MonteCarloPPZF),'-o','Color',"#EDB120",'LineWidth', 1.2);
