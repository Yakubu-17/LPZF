...
tau=[2 3 4 6 8 10];%Number of pilots per coherence block
Kd=K;
si_canc=0;
...
sum_MonteCarloMRT = zeros(K,length(tau));
sum_MonteCarloFZF = zeros(K,length(tau));
sum_MonteCarloPZF = zeros(K,length(tau));
sum_MonteCarloPPZF = zeros(K,length(tau));
sum_closedForm_MRT = zeros(K,length(tau));
sum_closedForm_FZF = zeros(K,length(tau));
sum_closedForm_PZF = zeros(K,length(tau));
sum_closedForm_PPZF = zeros(K,length(tau));
...
Td = xi*(1 - tau(id)/Tc); %Pre log factor.
[U,~,~]=svd(randn(tau(id),tau(id))); %generating othogonal pilot sequences.
pilot_book=sqrt(tau(id))*U;%Pilot book matrix
    ...
    for ic=1:lsf_experiment
     ...
     [S,W,Z,X,E_S_temp,tau_S_l] = functionUEgrouping(M,beta_matrix,Pilot_indices,tau(id),threshold); %UE grouping

     [rate_sim_MRT,rate_sim_FZF,rate_sim_PZF,rate_sim_PPZF] = functionSimulation(L,M,K,p_maxdB,ppdB,beta_matrix,Phii,tau(id),Pilot_indices,pilot_book,S,W,Z,X,E_S_temp,tau_S_l,experiment); %Sum rate 
...
disp(['Setup ' num2str(id) ' out of ' num2str(length(tau))]);
end
disp('Execution Complete !!!')

figure(2);
hold on;
plot(tau,mean(sum_MonteCarloMRT),'-ob','LineWidth', 1.2);
plot(tau,mean(sum_MonteCarloFZF),'-or','LineWidth', 1.2);
plot(tau,mean(sum_MonteCarloPZF),'-ok','LineWidth', 1.2);
plot(tau,mean(sum_MonteCarloPPZF),'-o','Color',"#EDB120",'LineWidth', 1.2);
legend('MRT','FZF','PZF','PPZF','interpreter','latex')
xlabel('Number of antennass $$\tau_p$$','interpreter','latex')
