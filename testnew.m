...
beta_matrix = beta_matrix ./ repmat(sum(beta_matrix,2), 1, K);  % Normalize

tol = 1e-8;  % Tolerance value (you might still need this)
thres = 1;
...
for l = 1:L
    ...
    for k = 1:K
        ...
        % Debugging Lines
        disp(['AP: ', num2str(l), ', k: ', num2str(k), ', Index: ', num2str(beta_des_ind(k)), ', beta_des(k): ', num2str(beta_des(k))]); 

        if temp1 >= thres - tol   % Change: using tolerance
            break;
        else
